import numpy as np
import pyhip.elementwise as elementwise
from pytools import memoize, memoize_method
import pyhip.driver as drv
from pyhip.compyte.array import (
...
#from pyhip.characterize import has_double_support
...
def _get_common_dtype(obj1, obj2):
    return _get_common_dtype_base(obj1, obj2, True)
...
def _create_vector_types():
    ...
    from pyhip.tools import get_or_register_dtype
...
def _splay_backend(n, dev):
    ...
    from pyhip.tools import DeviceData
...
def sum(a, dtype=None, stream=None, allocator=None):
    from pyhip.reduction import get_sum_kernel
...
def subset_sum(subset, a, dtype=None, stream=None, allocator=None):
    from pyhip.reduction import get_subset_sum_kernel
...
def dot(a, b, dtype=None, stream=None, allocator=None):
    from pyhip.reduction import get_dot_kernel
...
def subset_dot(subset, a, b, dtype=None, stream=None, allocator=None):
    from pyhip.reduction import get_subset_dot_kernel
...
def _make_minmax_kernel(what):
    def f(a, stream=None):
        from pyhip.reduction import get_minmax_kernel
...
def _make_subset_minmax_kernel(what):
    def f(subset, a, stream=None):
        from pyhip.reduction import get_subset_minmax_kernel
